...
from data.pddl.env.local_env import make_local_pddl_env
...
_LOCAL_PDDL_GAMES = {"barman", "tyreworld"}

INVALID_ACTION_OBSERVATION = (
    "The action is not valid and therefore takes no effect. "
    "Use `check valid actions` to list currently valid actions before trying another action."
)
...
class PDDLEnv(DynamicEnv):
    ...
    def set_env(self, configs: dict) -> tuple[str, str]: 
        ...
        if self.game_name in _GAME_TO_PDDLGYM:
            self.local_env = None
            self.env = pddlgym.make("PDDLEnv{}-v0".format(_GAME_TO_PDDLGYM[self.game_name]))
            self.env.fix_problem_index(problem_index)
        elif self.game_name in _LOCAL_PDDL_GAMES:
            self.env = None
            metadata = self._resolve_task_metadata(configs, problem_index)
            self.local_env = make_local_pddl_env(self.game_name, metadata)
        else:
            raise ValueError(
                f"Unsupported PDDL game '{self.game_name}'. "
                f"Supported games: {sorted(set(_GAME_TO_PDDLGYM) | _LOCAL_PDDL_GAMES)}."
            )
    ...
    def _resolve_task_metadata(self, configs: dict, problem_index: int) -> dict:
        if configs.get("subgoals") or configs.get("goal"):
            return configs
        import json
        from pathlib import Path

        data_path = Path(__file__).resolve().parents[1] / "test.jsonl"
        rows = [
            json.loads(line)
        ...
        matches = [
            row for row in rows
            if row.get("additional_info", {}).get("subtask") == self.game_name
        ]
        if problem_index < 0 or problem_index >= len(matches):
            raise IndexError(
                f"No local PDDL metadata for {self.game_name} problem_index={problem_index}"
            )
        row = matches[problem_index]
        return {
            "goal": row.get("goal"),
    ...
    def _reset(self):
        if self.local_env is not None:
            ...
            self.goal = self.local_env.goal_text
            self.init_obs = self.local_env.state_text()
            self.goal_literals = set(self.local_env.subgoals)
            self.last_obs = None
            self.infos = dict()
            self.infos["goal_literal"] = self.goal_literals
            ...
            self.reward = self.local_env.reward
            self.done = self.local_env.done
            self.won = self.done
            return
    ...
    def step(self, action: str) -> tuple[str, float, bool]:
        ...
        if stripped.startswith("think:") or stripped.startswith("think "):
            self._update_info(action, "(thinking step recorded)")
        ...
        if "look around" in stripped:
            if self.local_env is not None:
                obs = self.local_env.full_state_text()
            else:
        ...
        if self.local_env is not None:
            result = self.local_env.step(action)
            if result.valid:
                self._update_local(action, result.observation, result.reward, result.done)
                self.infos["action_is_valid"] = True
                return self._get_obs(), 1 if self.done else 0, self.done
            invalid_obs = (
                INVALID_ACTION_OBSERVATION
                if "not valid" in result.observation.lower()
                else f"{result.observation} {INVALID_ACTION_OBSERVATION}"
            )
            self._update_info(action, invalid_obs)
        ...
        if action_literal is not None:
            step_result = self.env.step(action_literal)
            # Handle both old (4 values) and new gym (5 values) return formats
            if len(step_result) == 5:
                obs_temp, reward, terminated, truncated, infos = step_result
                done = terminated or truncated
            else:
                obs_temp, reward, done, infos = step_result
            # print(reward)
            ...
            if obs_temp == self.last_obs: 
                obs = INVALID_ACTION_OBSERVATION
                self._update_info(action, obs)
                self.infos["action_is_valid"] = False
                return self._get_obs(), -1, self.done
            else:
    ...
    def preprocess_action(cls, action: str) -> str:
        ...
        lines = [line.strip() for line in action.strip().splitlines() if line.strip()]
        return lines[-1] if lines else action.strip()
    ...
    def _update_local(self, action, obs, reward, done):
        self.won = bool(done)
        self.steps += 1
        self.reward = reward
        self.done = bool(done)
    ...
    def _literal_to_text(self, literal):
        ...
        game_map = _GAME_PREDICATE_MAPS.get(self.game_name, {})
        predicate_format = game_map.get(predicate_name)
        if predicate_format is not None:
            objects_name = [str(obj.name) for obj in objects]
            return predicate_format.format(*objects_name)
        return predicate_name + " " + " ".join([str(obj.name) for obj in objects])
...
# Per-game predicate rendering — previously a single global dict with
# duplicated keys that silently overrode each other across games.
_BLOCKS_PREDICATES = {
    "on": "{} is on {}.",
    "clear": "{} is clear.",
...
_GAME_PREDICATE_MAPS = {
    "blocks": _BLOCKS_PREDICATES,
    "blockworld": _BLOCKS_PREDICATES,
    "gripper": _GRIPPER_PREDICATES,
    "barman": _BARMAN_PREDICATES,
    "tyreworld": _TYREWORLD_PREDICATES,
    "hanoi": _HANOI_PREDICATES,
}


# Legacy flat dict kept only as a fallback / reference; new code goes through
# `_GAME_PREDICATE_MAPS[game_name]` so duplicate keys across games no longer
...
description_map = {
    ...
    unstack <block>: allows the arm to unstack the clear block on top of another block if the arm is empty and that top block is clear. After the unstack action, the arm will be holding that block, it will no longer be on top of the block below, and the block below will become clear.
    ''',
    "barman": '''
