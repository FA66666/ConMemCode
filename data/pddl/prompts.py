...
- Output exactly one next environment action, not a full plan.
- If no valid-action constraint is present and the previous observation says an action was invalid, output `check valid actions` before trying another action.
- Before you respond, you must ensure that your output action strictly complies with the rules described above!
"""
...
- You should strictly follow these actions, no any other actions are allowed!
- Output exactly one next environment action, not a full plan.
- If the environment provides a valid-action list, copy one action from that list exactly. Otherwise use the compact command form shown above, for example `pick ball1 rooma right`.
- Do not pick up or move a ball that already satisfies its goal location unless it is blocking completion.
- Use the two grippers as a shuttle: pick up at most two unsatisfied balls in the current room, move to the room required by their goals, drop them, then return only if unsatisfied balls remain elsewhere.
- Before `pick <obj> <room> <gripper>`, verify the object is currently at that room and the gripper is free.
- Before `drop <obj> <room> <gripper>`, verify Robby is currently at that room and is carrying the object with that gripper.
- If no valid-action constraint is present and the previous observation says an action was invalid, output `check valid actions` before trying another action.
- Before you respond, you must ensure that your output action strictly complies with the rules described above!
"""
